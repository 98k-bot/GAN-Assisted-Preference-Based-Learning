...
class ComparisonRewardPredictor():
    ...
    def train_predictor(self):
        ...
        D_real = np.array(D_real).transpose(0, 1).reshape(64)
        D_fake = np.array(D_fake).transpose(0, 1).reshape(64)
...
def main():
    ...
    parser.add_argument('-e', '--env_id', default="Hopper-v1", type=str)
    ...
    parser.add_argument('-L', '--pretrain_labels', default=175, type=int)
    parser.add_argument('-t', '--num_timesteps', default=5e6, type=int)
    parser.add_argument('-a', '--agent', default="pposgd_mpi", type=str)
    parser.add_argument('-i', '--pretrain_iters', default=100, type=int)
